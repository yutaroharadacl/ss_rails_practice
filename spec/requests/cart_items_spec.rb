...
RSpec.describe 'CartItems', type: :request do
  let!(:store) { Store.create!(name: '店舗', code: 'CI-REQ-STORE') }
  let!(:product) { create_product!(store: store, name: 'りんご', code: 'CI-REQ-APPLE') }
  ...
  describe 'POST /cart_items' do
    ...
    it 'すでに存在する商品の場合、指定した数量でCartItemの数量が増加する' do
      post cart_items_path, params: { cart_item: { product_id: product.id, quantity: 2 } }
      post cart_items_path, params: { cart_item: { product_id: product.id, quantity: 3 } }
    ...
    it '保存に失敗したとき、cart_pathにリダイレクトする' do
      expect do
        post cart_items_path, params: { cart_item: { product_id: product.id, quantity: 0 } }
