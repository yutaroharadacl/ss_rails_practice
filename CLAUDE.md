...
## コードレビューで指摘された規約（このリポジトリ固有）

現場のレビューで指摘された、このリポジトリ・チーム固有の規約。一般的なRailsの知識（`find`と`find_by`の使い分け、`redirect_back`と`redirect_to`の違いなど）はここに含めない。

- **flashメッセージはハードコードせず `config/locales/flash.ja.yml` に定義する。** `flash.<controller>.<action>.notice` / `flash.<controller>.<action>.error.<key>` の階層で定義し、コントローラからは `t('flash....')` で呼び出す（例: `app/controllers/cart_items_controller.rb`）。
- **モデルのバリデーションエラーで表示される属性名は `config/locales/models.ja.yml` に論理名を定義する。** `activerecord.attributes.<model>.<attribute>` の形式（例: `cart_item.product_id: 商品ID`）で定義しておくと、物理名がそのままエラーメッセージに出るのを防げる。
- **`config/locales/*.yml` などのテキストファイルは末尾に改行を入れる。**
- **マイグレーションで `create_table` / `drop_table` を書く際は、`change` の代わりに `up` / `down` を使い、`data_source_exists?` で存在チェックしてから実行する。** rollbackや再実行時の安全性のため。
